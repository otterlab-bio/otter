...
- The immutable `run.yaml` is the single execution boundary for **both** executors. The snapshot records the executor it was resolved for, and `otter run --executor` must agree with that selection; a mismatch is refused rather than silently rerouted.
- Legacy `otter.yaml` is an authoring format only. It is refused by both executors and must be converted with `otter config migrate` and then resolved. Migration maps the compatibility track onto the Snakemake executor.
...
- Every declared reference must be named as `id@release`. A release label is mandatory because it pins the assembly and annotation provenance; a legacy filesystem path does not identify an immutable release.
- Changes to samples, references, workflow assets, executor, toolchain, or immutable resources require a new run.
- `otter init` pins the project's workflow assets and records their digests; `otter create` and `otter config migrate` write project intent. A canonical project needs both, which is why migration output must land in an `init`-created root.
...
- Site profiles are generated by `otter site generate`, which records the observable environment and requires the operator to supply the policy fields it cannot discover (partition, account, QOS).
- A generated profile is validated with the loader's own rules before it is written, and it is consumed: resolving with `--site <id>` and no `--reference-root` takes the registry root from the profile.
